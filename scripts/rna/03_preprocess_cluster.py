...
def main():
    input_file = Path("data/processed/rna/pbmc3k_qc.h5ad")
    output_file = Path("data/processed/rna/pbmc3k_processed.h5ad")
    ...
    # === PCA ===
    print("\n[4] Running PCA...")
    sc.tl.pca(adata, n_comps=N_PCS, random_state=RANDOM_STATE, return_info=True)
    print(f"   PCA shape: {adata.obsm['X_pca'].shape}")

    # === Variance plot - FIXED: use variance_ratio from uns ===
    fig, ax = plt.subplots(1, 1, figsize=(4, 3))
    variance_ratio = adata.uns['pca']['variance_ratio'][:N_PCS]
    ax.plot(range(1, N_PCS + 1), variance_ratio, 'o-')
    ax.set_xlabel('PC'); ax.set_ylabel('Variance Ratio')
    ax.set_title('PCA Variance Ratio')
    fig.tight_layout()
    fig.savefig("results/figures/rna/pca_variance.png", dpi=100)
    plt.close()
    print("   Saved: results/figures/rna/pca_variance.png")
    ...
    # === Leiden clustering ===
    print("\n[7] Leiden clustering...")
    sc.tl.leiden(adata, resolution=LEIDEN_RESOLUTION, random_state=RANDOM_STATE, flavor='igraph')
    ...
    fig.savefig("results/figures/rna/umap_leiden.png", dpi=100, bbox_inches='tight')
    plt.close()
    print("\n   Saved: results/figures/rna/umap_leiden.png")
